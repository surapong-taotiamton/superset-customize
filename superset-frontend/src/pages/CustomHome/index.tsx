...
import MyCustomPanel from 'src/components/MyCustomPanel';
...
function Welcome({ user, addDangerToast }: WelcomeProps) {
  ...
  const [newDashboardData, setNewDashboardData] = useState<Array<object> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getAllData('dashboard')
        .then(r => {
            console.log("Get all dashboard success", r)
            setNewDashboardData(r);
            return Promise.resolve();
        })
        .catch( (err: unknown) => {
            console.log("Get all dashboard fail", err)
            setNewDashboardData([]);
            addDangerToast(
